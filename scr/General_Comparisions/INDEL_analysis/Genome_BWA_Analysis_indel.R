########################################################################
##################Genome - BWA Analysis ################################ 
################## Reference Homo Analysis #############################

setwd("~/Desktop/New_analysis/INDEL_analysis/individual_sample_genomes/Variant_list/")
temp =list.files (pattern ="*whomo_g.txt")
...
Ref_Gen_Summary_indel =cbind(sample,number,mean_CD,mean_GQ,mean_MRR)
write.table(Ref_Gen_Summary_indel,"/Users/salendrapradh/WGS-and-WES/result/General Comparison/INDEL/Ref_Gen_Summary_indel.tsv",
            sep="\t", quote =F , row.names = F)

##################################################################################
############ Alternate Heterozugous Genome Analysis ########################

setwd("~/Desktop/New_analysis/INDEL_analysis/individual_sample_genomes/Variant_list")
temp =list.files (pattern ="*het_g.txt")
myfiles = lapply (temp,  read.table, sep = " ", stringsAsFactor=FALSE) 
sample <- sapply(strsplit(temp, "_"), "[",1)
...
Het_Gen_Indel_Summary =cbind(sample,number,mean_CD,mean_GQ,mean_MRR)
write.table(Het_Gen_Indel_Summary,"/Users/salendrapradh/WGS-and-WES/result/General Comparison/INDEL/Het_Gen_Indel_Summary.tsv",
            sep="\t", quote =F , row.names = F)

##################################################################################
############ Mutant Homozygous Genome Analysis ########################

setwd("~/Desktop/New_analysis/INDEL_analysis/individual_sample_genomes/Variant_list/")
temp =list.files (pattern ="*mhomo_g.txt")
myfiles = lapply (temp,  read.table, sep = " ", stringsAsFactor=FALSE) 
sample <- sapply(strsplit(temp, "_"), "[",1)
...
Homo_Gen_Indel_Summary =cbind(sample,number,mean_CD,mean_GQ,mean_MRR)
write.table(Homo_Gen_Indel_Summary,"/Users/salendrapradh/WGS-and-WES/result/General Comparison/INDEL/Homo_Gen_Indel_Summary.tsv",
            sep="\t", quote =F , row.names = F)

##################################################################################
############ Mutant homo second allele(2/2) case Genome Analysis ########################

setwd("~/Desktop/New_analysis/individual_sample_genomes_2/Variant_list")
temp =list.files (pattern ="*mhomo_second_allele_g.txt")
myfiles = lapply (temp,  read.table, sep = " ", stringsAsFactor=FALSE) 
sample <- sapply(strsplit(temp, "_"), "[",1)
test = vector ("list", length(sample))
names(test) <- sample
names(myfiles) <- sample
mean_CD= numeric(length=95)  #Since one file dib't have this kind of combination so 95 samples were taken
...
Mut_Second_Allele_Genome_Summary =cbind(sample,mean_CD,mean_GQ,mean_MRR)
write.table(Mut_Second_Allele_Genome_Summary,"~/Desktop/Compared/New_analysis/Mut_Second_Allele_Genome_Summary.tsv",
            sep="\t", quote =F , row.names = F)
...
setwd("~/Desktop/New_analysis/individual_sample_genomes_2/Variant_list")
temp =list.files (pattern ="*het_second_allele_g.txt")
myfiles = lapply (temp,  read.table, sep = " ", stringsAsFactor=FALSE) 
sample <- sapply(strsplit(temp, "_"), "[",1)
test = vector ("list", length(sample))
names(test) <- sample
names(myfiles) <- sample
mean_CD= numeric(length=95) 
mean_GQ = numeric(length=95)
mean_MRR = numeric(length=95)
...
Het_Second_Allele_Genome_Summary =cbind(sample,mean_CD,mean_GQ,mean_MRR)
write.table(Het_Second_Allele_Genome_Summary,"~/Desktop/Compared/New_analysis/Het_Second_Allele_Genome_Summary.tsv",
            sep="\t", quote =F , row.names = F)
...
setwd("~/Desktop/New_analysis/individual_sample_genomes_2/Variant_list")
temp =list.files (pattern ="*het_first_second_allele_g.txt")
myfiles = lapply (temp,  read.table, sep = " ", stringsAsFactor=FALSE) 
sample <- sapply(strsplit(temp, "_"), "[",1)
test = vector ("list", length(sample))
names(test) <- sample
...
Het_First_Second_Allele_Genome_Summary =cbind(sample,mean_CD,mean_GQ,mean_MRR)
write.table(Het_First_Second_Allele_Genome_Summary,"~/Desktop/Compared/New_analysis/Het_First_Second_Allele_Genome_Summary.tsv",
            sep="\t", quote =F , row.names = F)

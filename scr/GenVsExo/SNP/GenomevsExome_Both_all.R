########################################################################
################## Both GenvsExo Analysis ##############################
## (Gen vs Exo) summary report #########################################

setwd("/Users/salendrapradh/Documents/GenvsExo/Both/")
...
Summary_Both_GenVSExome = cbind (sample,number, mean_CD, mean_GQ, mean_MRR)
write.table(Summary_Both_GenVSExome,"/Users/salendrapradh/WGS-and-WES/result/GenvsExo/Summary_Both_GenVSExome.tsv",
            sep="\t", quote =F , row.names = F)


################## Reference (0/0) Homo Analysis #############################
setwd("/Users/salendrapradh/Documents/GenvsExo/Both/Con_Dis/")
...
for (i in 1:96){
  ...
  number[i] <- length(CD_ALT_BWT)
  test_Ref[[i]][[1]] = cbind (CD_Ref_BWT,CD_BWT,MRR_BWT,CD_BWT,GQ_BWT)
  test_Ref[[i]][[2]] = cbind (CD_Ref_BWA,CD_BWA,MRR_BWA,CD_BWA,GQ_BWA)
}
...
Reference_Both_GenvsExo_Summary = cbind(sample,number,mean_CD,mean_GQ,mean_MRR)
write.table(Reference_Both_GenvsExo_Summary,
            "/Users/salendrapradh/WGS-and-WES/result/GenvsExo/Reference_Both_GenvsExo_Summary.tsv",
            sep="\t", quote =F , row.names = F)
...
Het_Both_GenvsExo_Summary =cbind(sample,number,mean_CD,mean_GQ,mean_MRR)
write.table(Het_Both_GenvsExo_Summary,
            "/Users/salendrapradh/WGS-and-WES/result/GenvsExo/Het_Both_GenvsExo_Summary.tsv",
            sep="\t", quote =F , row.names = F)
...
Mut_Both_GenvsExo_Summary=cbind(sample,number,mean_CD,mean_GQ,mean_MRR)
write.table(Mut_Both_GenvsExo_Summary,
            "/Users/salendrapradh/WGS-and-WES/result/GenvsExo/Mut_Both_GenvsExo_Summary.tsv",
            sep="\t", quote =F , row.names = F)
...
#######################################################################
setwd("/Users/salendrapradh/Documents/GenvsExo/Both/Con_Dis/")
temp =list.files (pattern ="*discordant_both_WES_WGS.txt")
myfiles = lapply (temp,  read.table, sep = " ", stringsAsFactor=FALSE) 
sample <- sapply(strsplit(temp, "_"), "[",1)
...
## test with 2 for Exome and 3 for Genome

test_BWA_ExoGen = vector ("list", length(sample))
names(test_BWA_ExoGen) <- sample
...
for (i in 1:96){
    for (j in 1:6){
      ...
      test_BWA_ExoGen[[i]][[1]] = cbind (CD_Ref_BWT,CD_ALT_BWT,MRR_BWT,CD_BWT,GQ_BWT)
      test_BWA_ExoGen[[i]][[2]] = cbind (CD_Ref_BWA,CD_ALT_BWA,MRR_BWA,CD_BWA,GQ_BWA)
    }
}
...
write.table(Summary_discordant_GenvsExo,
            "/Users/salendrapradh/WGS-and-WES/result/GenvsExo/Summary_discordant_GenvsExo.tsv",
            sep="\t", quote =F , row.names = F)
